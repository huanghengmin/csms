package com.hzih.ra.web.filter;
...
//		} else 
		...
		if(SessionUtils.getAccount((HttpServletRequest) request) == null && requestURL.indexOf("login") < 0
                /*&&requestURL.indexOf("publicUser.jsp")<0&&
                requestURL.indexOf("publicDevice.jsp")<0*/){
			response.sendRedirect(request.getContextPath() + "/login.jsp");
		}else {
			// 如果不是前台自动发起的请求，则重新设置logintime
			if (requestURL.indexOf("checkTimeout.action") < 0
                    && requestURL.indexOf("DoTermStatusAll")<0
                    && requestURL.indexOf("DoTermStatus")<0
                    && requestURL.indexOf("alertRule.action") < 0
                    && requestURL.indexOf("alert.action") < 0) {
