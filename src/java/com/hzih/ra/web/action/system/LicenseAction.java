package com.hzih.ra.web.action.system;
    ...
    public String uploadRemote() throws Exception {
        ...
        try {
            if("license".equals(uploadFileFileName)){
                ...
                boolean isUpdate = ecu.updateLicense(org.aspectj.util.FileUtil.readAsString(uploadFile));
                if (isUpdate) {
                    msg = "上传成功!";
                } else {
                    msg = "上传失败";
            ...
            logger.error("证书管理", e);
            logService.newLog("ERROE", SessionUtils.getAccount(request).getUserName(), "证书管理","用户上传远程许可证信息失败");
            msg = "上传失败";
        }
        json = "{'success':true,'msg':'"+msg+"'}";
        base.actionEnd(response, json ,result);
