Ext.onReady(function () {
    ...
    var proxy = new Ext.data.HttpProxy({
        url:"../../WhiteListAction_findByPages.action" ,
        timeout: 10*1000
    });
        ...
        items:[
            {
                ...
                iconCls:'add',
                handler:function () {
                    addWhiteAddress(grid_panel, store);
                }
            }]
    });
...
/*    var typeData = [
    ...
    var tbar = new Ext.Toolbar({
        ...
        autoHeight:true,
        items:[
            '授权访问URL地址', new Ext.form.TextField({
                name : 'url',
...
//    if(r.get("status")=="0"){
        return String.format(
            '<a id="deleteWhiteAddress.info" href="javascript:void(0);" onclick="deleteWhiteAddress();return false;" style="color: green;">删除</a>&nbsp;&nbsp;&nbsp;'  +
                '<a id="updateWhiteAddress.info" href="javascript:void(0);" onclick="updateWhiteAddress();return false;" style="color: green;">更新</a>&nbsp;&nbsp;&nbsp;' /* +
                '<a id="joinBlackList.info" href="javascript:void(0);" onclick="joinBlackList();return false;">加入黑名单</a>&nbsp;&nbsp;&nbsp;'*/
        );
//    }else if(r.get("status")=="1"){
//        return String.format(
//            '<a id="deleteRaUser.info" href="javascript:void(0);" onclick="deleteAccessAddress();return false;">删除</a>&nbsp;&nbsp;&nbsp;'  +
//                '<a id="updateRaUser.info" href="javascript:void(0);" onclick="updateAccessAddress();return false;">更新</a>&nbsp;&nbsp;&nbsp;'/*  +
...
//    if(r.get("status")=="0"){
//        return String.format('<font color="red" >仅当前地址允许访问</font>');
//    }else if(r.get("status")=="1"){
//        return String.format('<font color="#4169e1" >服务下所有地址皆允许访问</font> ');
//    }
//}

function deleteWhiteAddress() {
    ...
    if(!recode){
        ...
        Ext.Msg.confirm("提示", "确认删除吗？", function(sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url : "../../WhiteListAction_delete.action",
                    timeout: 20*60*1000,
                    method : "POST",
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items:[
                ...
                listeners:{
                    blur:function(){
                        ...
                        if(count>1){
                            ...
                            Ext.Ajax.request({
                                url : '../../WhiteListAction_checkUrl.action',
                                timeout: 20*60*1000,
                                method : 'post',
                                params : {
                                    url : url
            ...
            new Ext.form.ComboBox({
                ...
                hiddenName:"whiteList.status",
                id:'hzih.update.whiteList.status',
                store: allDataStore,
                valueField: 'id',   //下拉框具体的值（例如值为SM，则显示的内容即为‘短信’）
                displayField: 'name'//下拉框显示内容
            })*//*,
                ...
                handler:function(){
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url :'../../WhiteListAction_modify.action',
                            timeout: 20*60*1000,
                            method :'POST',
                            waitTitle :'系统提示',
                            params:{
...
/*function joinWhiteList(){
    ...
    if(!recode){
        ...
        Ext.Msg.confirm("提示", "确认恢复到白名单吗？", function(sid) {
            if (sid == "yes") {
                Ext.Ajax.request({
                    url : "../../WhiteListAction_joinWhiteList.action",
                    timeout: 20*60*1000,
                    method : "POST",
                    params:{url:recode.get("url")},
                    success : function(form, action) {
...
function addWhiteAddress(grid_panel, store){
   /* var typeAddData = [
        [0,'白名单地址'],[1,'黑名单地址']
    ];
        ...
        bbar:[
            '->',
            {
                ...
                handler:function(){
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url :'../../WhiteListAction_add.action',
                            timeout: 20*60*1000,
                            method :'POST',
                            waitTitle :'系统提示',
                            waitMsg :'正在连接...',
